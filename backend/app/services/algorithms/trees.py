...
def serialize_tree_node(node: Optional[Any]) -> Optional[Dict[str, Any]]:
    if not node:
        return None
    return {
        'id': getattr(node, 'id', None),
        'val': getattr(node, 'val', getattr(node, 'char', None)),
        'height': getattr(node, 'height', None),
        'balance_factor': getattr(node, 'balance_factor', None),
        'color': getattr(node, 'color', None),
        'interval': getattr(node, 'interval', None),
        'is_end_of_word': getattr(node, 'is_end_of_word', False),
        'word': getattr(node, 'word', ''),
        'left': serialize_tree_node(getattr(node, 'left', None)),
        'right': serialize_tree_node(getattr(node, 'right', None)),
        'children': {k: serialize_tree_node(v) for k, v in getattr(node, 'children', {}).items()} if hasattr(node, 'children') and isinstance(node.children, dict) else None
    }
...
def bst_operations_with_steps(
    ...
    def delete_node(node: Optional[BSTNode], val: int) -> Optional[BSTNode]:
        if not node:
            return None
        if val < node.val:
    ...
    # 1. Build initial tree from history or initial_values
    if operation_history:
        for item in operation_history:
            item_op = item.get("op")
            item_val = item.get("value") if item.get("value") is not None else item.get("val")
            if item_op == "insert" and item_val is not None:
                root = insert(root, int(item_val))
            elif item_op == "delete" and item_val is not None:
                root = delete_node(root, int(item_val))
    elif initial_values:
    ...
    if root is None:
        steps.append({
            ...
            'edges': [],
            'active_node_id': None,
            'message': '🌲 BST initialized as empty tree (root is null). Ready for Insert operations.'
        })
    else:
        steps.append({
            'step': 0,
            ...
            'message': f'🌲 BST loaded with {len(current_nodes)} nodes. Root is {root.val} (depth 0). Traversal begins at Root.'
        })
    ...
    if op == 'insert' and value is not None:
        if root is None:
            id_counter += 1
            root = BSTNode(value, f"node-{id_counter}")
            ...
            steps.append({
                ...
                'active_node_id': root.id,
                'message': f'🎯 Inserted {value} as the Root of the BST.'
            })
        else:
            curr = root
            parent = None
        ...
        if root is None:
            steps.append({
                ...
                'edges': [],
                'active_node_id': None,
                'message': f'Tree is empty. Value {value} not found.'
            })
        else:
            curr = root
            parent = None
    ...
    return steps, metrics, serialize_tree_node(root)
...
def avl_operations_with_steps(
    ...
    def avl_insert(node: Optional[AVLNode], val: int, record_steps: bool = False) -> AVLNode:
        nonlocal id_counter
        if not node:
            id_counter += 1
        ...
        if val < node.val:
            node.left = avl_insert(node.left, val, record_steps)
        elif val > node.val:
            node.right = avl_insert(node.right, val, record_steps)
        else:
            return node
        ...
        # LL Case
        if bf > 1 and val < (node.left.val if node.left else 0):
            if record_steps:
                steps.append({
                    'step': len(steps),
        ...
        # RR Case
        if bf < -1 and val > (node.right.val if node.right else 0):
            if record_steps:
                steps.append({
                    'step': len(steps),
    ...
    def avl_delete(node: Optional[AVLNode], val: int, record_steps: bool = False) -> Optional[AVLNode]:
        if not node:
            return None

        if val < node.val:
            node.left = avl_delete(node.left, val, record_steps)
        elif val > node.val:
            node.right = avl_delete(node.right, val, record_steps)
        else:
            if not node.left or not node.right:
                ...
                while succ.left:
                    succ = succ.left
                node.val = succ.val
                node.right = avl_delete(node.right, succ.val, record_steps)
        ...
        if bf > 1 and get_balance(node.left) >= 0:
            if record_steps:
                steps.append({
                    ...
                    'active_node_id': node.id,
                    'rotation': 'LL',
                    'message': f'Post-delete imbalance at node {node.val} (BF = {bf} > 1, Left-Left case). Performing single RIGHT rotation.'
                })
            return right_rotate(node)
        ...
        if bf > 1 and get_balance(node.left) < 0:
            if record_steps:
                steps.append({
                    ...
                    'active_node_id': node.id,
                    'rotation': 'LR',
                    'message': f'Post-delete imbalance at node {node.val} (BF = {bf} > 1, Left-Right case). Double rotation: Left on {node.left.val} then Right on {node.val}.'
                })
            node.left = left_rotate(node.left)
            return right_rotate(node)
        ...
        if bf < -1 and get_balance(node.right) <= 0:
            if record_steps:
                steps.append({
                    ...
                    'active_node_id': node.id,
                    'rotation': 'RR',
                    'message': f'Post-delete imbalance at node {node.val} (BF = {bf} < -1, Right-Right case). Performing single LEFT rotation.'
                })
            return left_rotate(node)
        ...
        if bf < -1 and get_balance(node.right) > 0:
            if record_steps:
                steps.append({
                    ...
                    'active_node_id': node.id,
                    'rotation': 'RL',
                    'message': f'Post-delete imbalance at node {node.val} (BF = {bf} < -1, Right-Left case). Double rotation: Right on {node.right.val} then Left on {node.val}.'
                })
            node.right = right_rotate(node.right)
            return left_rotate(node)
    ...
    root_ref: List[Optional[AVLNode]] = [None]

    # Replay history or build from initial_values
    if operation_history:
        for item in operation_history:
            item_op = item.get("op")
            item_val = item.get("value") if item.get("value") is not None else item.get("val")
            if item_op == "insert" and item_val is not None:
                root_ref[0] = avl_insert(root_ref[0], int(item_val), record_steps=False)
            elif item_op == "delete" and item_val is not None:
                root_ref[0] = avl_delete(root_ref[0], int(item_val), record_steps=False)
    elif initial_values:
        for v in initial_values:
            root_ref[0] = avl_insert(root_ref[0], v, record_steps=False)
    ...
    if root_ref[0] is None:
        steps.append({
            ...
            'edges': [],
            'active_node_id': None,
            'message': '🌲 AVL Tree initialized as empty tree (root is null). Ready for Insert operations.'
        })
    else:
        steps.append({
            'step': 0,
            ...
            'active_node_id': root_ref[0].id,
            'message': f'🌲 AVL Tree loaded with {len(init_nodes)} nodes. Root is {root_ref[0].val} (BF = {root_ref[0].balance_factor}). Traversal begins at Root.'
        })

    if op == 'insert' and value is not None:
        if root_ref[0] is None:
            id_counter += 1
            root_ref[0] = AVLNode(value, f"avl-{id_counter}")
            update_meta(root_ref[0])
            ...
            steps.append({
                ...
                'active_node_id': root_ref[0].id,
                'message': f'🎯 Inserted {value} as the Root of the AVL tree.'
            })
        else:
            curr = root_ref[0]
            while curr:
            ...
            root_ref[0] = avl_insert(root_ref[0], value, record_steps=True)
            final_nodes = _compute_tree_layout(root_ref[0])
            steps.append({
                'step': len(steps),
                'event_type': 'balanced',
                ...
                'message': f'🎯 Value {value} inserted. Tree balanced with maximum height {get_height(root_ref[0])}.'
            })
        ...
        if root_ref[0] is None:
            ...
            root_ref[0] = avl_delete(root_ref[0], value, record_steps=True)
            final_nodes = _compute_tree_layout(root_ref[0])
            steps.append({
                'step': len(steps),
                'event_type': 'deleted',
                ...
                'edges': copy.deepcopy(_get_tree_edges(final_nodes)),
                'active_node_id': root_ref[0].id if root_ref[0] else None,
                'message': f'✓ Value {value} deleted from AVL Tree and rebalanced.'
            })
        ...
        while curr:
            ...
            if value == curr.val:
                ...
                steps.append({
                    ...
                    'edges': copy.deepcopy(_get_tree_edges(_compute_tree_layout(root_ref[0]))),
                    'active_node_id': curr.id,
                    'message': f'🎯 Found target {value} in AVL Tree!'
                })
                break
            elif value < curr.val:
                curr = curr.left
        ...
        if not found:
            steps.append({
                ...
                'edges': copy.deepcopy(_get_tree_edges(_compute_tree_layout(root_ref[0]))),
                'active_node_id': None,
                'message': f'❌ Target {value} is not present in the AVL Tree.'
            })
    ...
    return steps, metrics, serialize_tree_node(root_ref[0])
...
def red_black_with_steps(
    ...
    if operation_history:
        for item in operation_history:
            ...
            item_val = item.get("value") if item.get("value") is not None else item.get("val")
            if item_op == "insert" and item_val is not None:
                tree.insert(int(item_val), record_steps=False)
            elif item_op == "delete" and item_val is not None:
                tree.delete(int(item_val), record_steps=False)
    elif initial_values:
        for v in initial_values:
            tree.insert(v, record_steps=False)
    ...
    if tree.root == tree.NIL:
        steps.append({
            ...
            'edges': [],
            'active_node_id': None,
            'message': '🌲 Red-Black Tree initialized as empty tree (root is null). Ready for Insert operations.'
        })
    else:
        steps.append({
            'step': 0,
            ...
            'nodes': copy.deepcopy(layout),
            'edges': copy.deepcopy(_get_tree_edges(layout)),
            'active_node_id': tree.root.id,
            'message': f'🌲 Red-Black Tree loaded with {len(layout)} nodes. Root is {tree.root.val} (BLACK). Traversal begins at Root.'
        })
    ...
    if op == 'insert' and value is not None:
        ...
        if tree.root == tree.NIL:
            ...
            steps.append({
                ...
                'edges': copy.deepcopy(_get_tree_edges(layout)),
                'active_node_id': tree.root.id if tree.root != tree.NIL else None,
                'message': f'✓ Node {value} deletion finished with full double-black resolution. Root is {tree.root.val if tree.root != tree.NIL else "empty"}.'
            })
        ...
        while curr != tree.NIL:
            ...
            steps.append({
                ...
                'edges': copy.deepcopy(_get_tree_edges(layout)),
                'active_node_id': curr.id,
                'message': f'{"🌲 Starting search at Root node " if is_root else "Probing node "}{curr.val} ({curr.color}): comparing with target {value}.'
            })
            if value == curr.val:
                found = True
                steps.append({
                    ...
                    'edges': copy.deepcopy(_get_tree_edges(layout)),
                    'active_node_id': curr.id,
                    'message': f'🎯 Found target {value} in Red-Black Tree!'
                })
                break
            elif value < curr.val:
                curr = curr.left
        ...
        if not found:
            steps.append({
                ...
                'edges': copy.deepcopy(_get_tree_edges(layout)),
                'active_node_id': None,
                'message': f'❌ Target {value} is not present in the Red-Black Tree.'
            })

    final_layout = rb_to_layout(tree.root)
    metrics = {
        'total_nodes': len(final_layout),
        'tree_type': 'redblack'
    }
    return steps, metrics, serialize_tree_node(tree.root if tree.root != tree.NIL else None)
...
def trie_operations_with_steps(
    ...
    # Replay history or load words
    vocab = []
    if operation_history:
        for item in operation_history:
            item_op = item.get("op")
            w = item.get("word") or item.get("query")
            if item_op == "insert" and w:
                insert_word(w)
                vocab.append(w)
    elif words:
        for w in words:
    ...
    steps.append({
        ...
        'active_node_id': root.id,
        'message': f'🌲 Trie loaded with vocabulary: {", ".join(vocab) if vocab else "Empty"}. ROOT is the prefix entry point.'
    })

    if op == 'insert' and query:
        insert_word(query)
        layout = layout_trie(root)
        edges = get_trie_edges(layout)
        steps.append({
            ...
            'edges': copy.deepcopy(edges),
            'active_node_id': root.id,
            'message': f'🎯 Word "{query}" inserted into Trie dictionary.'
        })

    elif (op == 'search' or op == 'autocomplete') and query:
        curr = root
        matched = True
        path_ids = [root.id]
...
def segment_tree_with_steps(
    ...
    array = list(arr) if arr else [5, 2, 8, 6, 3, 7]
    if operation_history:
        for item in operation_history:
            if item.get("op") == "update" and "index" in item and "value" in item:
                u_idx = item["index"]
                if 0 <= u_idx < len(array):
                    array[u_idx] = item["value"]
    ...
    def build_tree(node_idx: int, L: int, R: int) -> SegNode:
        if L == R:
            tree[node_idx] = array[L]
            return SegNode(node_idx, L, R, array[L])
        mid = (L + R) // 2
    ...
    steps.append({
        ...
        'message': f'🌲 Segment Tree built for RMQ over array {array}. Root interval is [0, {n-1}] with aggregate minimum = {root_seg.val}.'
    })
    ...
    return steps, {'total_nodes': len(layout), 'tree_type': 'segment'}, serialize_tree_node(root_seg)
...
def fenwick_tree_with_steps(
    ...
    array = list(arr) if arr else [3, 2, -1, 6, 5, 4, -3, 3]
    n = len(array)
    bit = [0] * (n + 1)
    steps = []
    ...
    return steps, {'bit_size': len(bit), 'tree_type': 'fenwick'}, None
